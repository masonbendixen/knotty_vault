...
- [x] **Pin bumped 8/3/2026** — Mason pushed honuware (CI green) at `c258a8c1776e32932ff0c89a0d0c4d3253891c75` ("Blog support server side"); the app's top-level `CMakeLists.txt` `GIT_TAG` moved from `3b1c3dc` to that SHA, verified against `origin/HEAD` via `ls-remote` before editing. Then re-verified end to end: a from-scratch **pinned-SHA build passed all 4727 tests** against the cloned commit.
