...
>
> **§6.3 UI rework (2026-05-29):** The first cut of `ClassScheduleManageComponent` was rejected by the user ("the UI is terrible … doesn't look like anything else in the website"). Reworked to match the admin look-and-feel (e.g. `product-list`): Angular Material throughout (`mat-card` with the project's `1px solid #d1d5db` border, `mat-table`, `mat-raised`/`mat-stroked`/`mat-icon-button`, `mat-form-field`/`mat-select`, `mat-spinner`, back-link + page-header). Terminology switched from jargon ("run"/"implementation") to the **formatted database table names**: *Class Instances*, *Class Schedules*, *Class Schedule Slots*. All actions are real buttons (Open / Deactivate / Migrate / Edit slots / Add / Delete). IDs are joined to friendly names — product via `getTableRows('products')`, facility/room via the existing lookup maps, instructor via `getInstructors()`. Slots now show **day + time + duration + facility + room + instructor**, and existing slots for the selected schedule are loaded via `getFilteredTableRows('class_schedule_slots', …)` (see §6.3 note), closing the prior "added-only" gap.
...
- [x] Consolidated into one `ClassScheduleManageComponent` (Material, matching `product-list`): a **Class** `mat-select` → **Class Instances** `mat-table` with **Open** + **Deactivate** + an **Add class instance** form (product `mat-select`) + **Migrate to a new product**; → **Class Schedules** table with **Edit slots** + **Deactivate** + an **Add class schedule** form (priority dropdown, "copy slots from class schedule" select); → **Class Schedule Slots** table (day / time / duration / facility / room / instructor) with **Add slot** (day-of-week dropdown, HH:MM `<input type="time">` per OQ-CSI-7, duration, facility/room/instructor dropdowns, optional capacity) + **Delete slot**, rejecting duplicate tuples (`DUPLICATE_SLOT`).
- [x] **Schedule-on-date preview**: date picker → `getClassSchedulePreview` → active instance + schedule + resolved slot `mat-table`.
...
- [x] No materialize dialog. Component spec (26 tests) covers all flows and passes headless.
- **Backend gap (mitigated):** there is still no dedicated "list slots for a schedule" endpoint, but `class_schedule_slots` is a registered nested CRUD table, so the slot section now lists existing slots via `getFilteredTableRows('class_schedule_slots', 'day_of_week', …, [{class_schedule_id}])` (and optimistically appends on add). A dedicated typed list-slots endpoint remains a nice-to-have follow-up.
