...
- [x] No materialize dialog. **57 specs** (main component + 4 dialogs + date util) pass headless; `ng build` compiles clean.
- **Backend gap (mitigated):** there is still no dedicated "list slots for a schedule" endpoint, but `class_schedule_slots` is a registered nested CRUD table, so slots are listed via `getFilteredTableRows('class_schedule_slots', 'day_of_week', …, [{class_schedule_id}])`, reloaded after each slot mutation. A dedicated typed list-slots endpoint remains a nice-to-have follow-up.
