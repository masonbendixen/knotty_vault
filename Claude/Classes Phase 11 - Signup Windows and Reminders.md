...
**Final model (redesigned 2026-06-19 per Mason):** workshops are booked **per occurrence from the class page**, not as a generic catalog product. There is no inline RSVP and no dateless Shop entry — clicking a session date opens that occurrence's booking page.
...
- [x] Clicking → `onBookOccurrence` navigates to **`/shop/class-occurrence`** (`ClassOccurrenceBookingComponent`) via router state `{slot, occurrence, className, startUs, amountCents, currency}` (included passes `amountCents:0`); logged-out → `/login`.
- [x] **`ClassOccurrenceBookingComponent`** materializes+books (`bookClassOccurrence`); **paid** → `PaymentMethodComponent` + `purchasePayCard`; **free ($0)** → skips payment and confirms ("Reserve your spot"). Booking is created once (kept across a payment retry). Reads occurrence from `history.state`; `invalid` state when navigated to directly.
- [x] **Removed the generic dateless Shop entry:** `catalog.component` now filters out `kind === 'class'` products (alongside `bookable_service`/`event`), so a workshop only appears via the class pages. (`CatalogProduct.kind` union gained `'class'`.)
- [x] **Surfaces to the booking flow now link to the class page** (`/classes/:id`): Our Schedule slot cards, Our Classes cards (already), and the Upcoming Workshops & Series offering titles.
- [x] Refund note on the checkout page points at the product's cancellation policy (`cancellation_policy_id`, resolved §11.3). *(Inline resolved-policy text would need it added to the payload — polish follow-up; the cancel path already honors it.)*
...
- [x] Specs — `class-detail.component.spec.ts` (non-workshop not clickable, paid click navigates with state, included free-reserve click, logged-out → /login, members-only not bookable), `class-occurrence-booking.component.spec.ts` (invalid, summary/price/UTC, book→pay→success, **free→no payment**, payment-error stays bookable, retry doesn't re-book), `catalog.component.spec.ts` (class filtered), `our-schedule` + `upcoming-offerings` link tests. **Verified green.**
