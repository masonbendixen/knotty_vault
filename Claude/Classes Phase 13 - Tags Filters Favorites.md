...
### 4.8 Scheduled job (resolved OQ-P13-2) ✅ (2026-06-30)
- [x] Added the **daily** `send_favorite_instructor_schedule_alerts` job (`POST /api/admin/send_favorite_instructor_schedule_alerts`) to `knottyyoga_helper`, wired in the three standard places: `scheduler/scheduled_job.cpp` `BuildStandardJobs` (`AppendIfEnabled` after `send_signup_open_reminders`), `JobIntervals::favoriteInstructorAlertSeconds` default **86400s (daily)** in `scheduler_config.h`, and a `--favorite_instructor_alert_interval` flag (default 86400) in `scheduler/main.cpp` — including the `BuildConfigFromFlags` mapping and the `LogConfigSummary` line. Idempotent + self-gating (sent-log + 24h throttle), so daily is correct; `<= 0` disables it like every other job.
- [x] **Permission:** the cron gates on `manage_class_schedule`, which the scheduler service account already holds (granted alongside the other class crons in `service_account.cpp` §3b) — extended that comment to list the new cron. No service-account change needed.
- [x] **Tests** (`scheduled_job_test.cpp`): job count 17→**18** (and the zero-disables-two count 15→**16**); a path spot-check for the new job; the all-zero aggregate-init extended to **18** explicit zeros (a trailing omitted member would otherwise keep its non-zero default member initializer and stay enabled); plus `FavoriteInstructorAlertCanBeDisabled` and `IntervalFromConfigPropagatedForFavoriteInstructorAlert`. **Backend `knottyyoga_tests` to be built + run by Mason.**
- *(`ValidateSchedulerConfig` validates only the original 10 intervals; the newer jobs incl. signup_reminders aren't validated either, so — consistent with that sibling — the new interval isn't added there.)*

### 4.9 Manual Test Guide — Favorite Instructors (live server, website only)

The favorite heart only appears when you are signed in.

**Sign in first.**
1. In the top navigation bar, click **Sign In**.
2. On the **Log In** page: in the **Email** field type your email; in the **Password** field type your password; click **Sign in**.

**Task 1 — Favorite an instructor.**
1. In the top navigation bar, click **About**, then in the dropdown click **Instructors**.
2. On the **Instructors** page, on the instructor's card, click the outlined heart button in the card's top-right corner (its tooltip reads **Add to favorites**).
3. The heart turns solid red and its tooltip becomes **Remove from favorites**.

**Task 2 — View your favorited instructors.**
1. In the top navigation bar, click the menu showing your first name (far right), then in the dropdown click **Profile**.
2. On the account page, click the **Favorite Instructors** card (heart icon; text "Instructors you follow and get notified about").
3. The **Favorite Instructors** page lists each instructor you favorited with their photo, name, and bio. With none favorited it reads "You haven't favorited any instructors yet…".

**Task 3 — Remove a favorite.**
1. On the **Favorite Instructors** page, on the instructor's row click the **Remove** button.
2. The row disappears immediately.

**Task 4 — Confirm it round-trips.**
1. On the **Favorite Instructors** page, click **← Back to account**.
2. In the top navigation bar, click **About**, then **Instructors**.
3. The heart on that instructor's card is outlined again (not favorited).

*Note:* the daily "your favorite instructor is teaching" alert email has no on-screen control — it runs as a server-side scheduled job (§4.8), so it cannot be exercised by clicking through the website.
