...
### 3.3 Backend
**Status (2026-06-25): class-occurrence core DONE; standalone events + service bookings are the remaining increment (see below).** Workshops and series are just classes with a `kind`, so the single derived-session walk already covers recurring + workshop + series — the bulk of the calendar and exactly what unblocks screenshots. New files: `business_logic/scheduling/calendar_helper.{h,cpp,_test.cpp}`, `endpoints/get_calendar.{h,cpp,_test.cpp}`, a `CalendarItem` converter in `scheduling_key_value_table.*`, wired into `web_app.cpp` + both `CMakeLists.txt`.

- [x] **Reuse, don't reinvent.** `CalendarHelper::GetCalendar(tx, fromUs, toUs, personId, facilityId)` composes per active class via the existing `ClassScheduleHelper::GetDerivedSessionsForRange` (walks dates, resolves active instance+impl per day, expands slots, left-joins persisted `event_sessions` for cancellations/subs). Cancelled occurrences dropped; effective (substitute-aware) instructor + first-tag `chip_color` resolved per item.
- [x] **New endpoint** `GET /api/calendar?from_us=&to_us=[&facility_id=]` → `{ items: [...] }`. Each item: `kind` (recurring→`class`, else `workshop`/`series`), `title`, `start_us`, `end_us`, `class_id`, `class_schedule_slot_id`, `occurrence_date_us`, `session_id` (0 until materialized), `facility_id`, `facility_name`, `instructor_name`, `chip_color`, + access fields. `from_us` clamped to **≥ now** (today-forward, OQ-P13-8); `to_us` defaults to ~6 weeks out; `facility_id` optional filter (OQ-P13-7). Auth-optional (reads the session if present; anonymous gets the full public schedule with empty access state).
- [x] **Per-viewer access resolution (resolved OQ-P13-5)** — when `personId > 0`, each class item carries `access`: `eligible` (passes the gate) / `members_only` (+ `required_permission_id`/`required_permission_name` for the buy-membership deep-link) / `needs_skill` (+ comma-joined `missing_skill_ids` for the skill popup). Classification reuses `ClassAccessHelper::CheckAccess` + `GetClassRequirements`; a skill-only failed group takes precedence over a membership gate (skills can't be purchased away). Anonymous → access empty.
- [x] Helper tests (`calendar_helper_test.cpp`, 18 cases): derivation + fields, workshop/series kind mapping, chip color (lowest sort_order wins / empty), facility filter, today-forward clamp, empty-past-range, cancelled excluded, sort order, the three access states + open-class + has-skill + skill-precedence, substitution-aware instructor, inactive-hidden. Converter tests (`scheduling_key_value_table_test.cpp`, 4 cases). Endpoint tests (`get_calendar_test.cpp`, 4 cases: empty, anonymous-no-access, facility filter, authenticated members_only + permission id/name).
- *Note (deferred to §3.3 follow-up, not yet wired):* **per-viewer visibility + signup-window state** ("Sign-ups open on …") is not yet layered on; it lands with the events/series booking-state work below. `eligible` currently means "passes the access gate" (paid-workshop price resolution happens at the booking page, as today).
- [ ] **Remaining increment — standalone events + service bookings.** Add `kind:"event"` items from `visible_event_sessions` (event_sessions with no `class_id`) and, for logged-in callers, `kind:"service_booking"` items from the viewer's service `my_bookings` (with `booking_id`). Plus signup-window state on future paid offerings. (These are independent composers in `CalendarHelper`; the `CalendarItem` struct already reserves `session_id`/`booking_id`/`kind`.)
