...
- [x] No new business logic to build the page — it reuses Phase 10's `GET /api/admin/instructor_load` / `getInstructorLoad`.
- [x] **BUT live testing surfaced a Phase 10 bug in `StaffingHelper::GetInstructorLoad`, fixed 2026-07-02 (#1 + #2):** the query INNER-JOINed `event_session_staffing`, which is written **only** by instructor substitutions (`instructor_substitution_helper.cpp`) and shift-trades (`shift_change_helper.cpp`) — never by normal scheduling / booking / check-in. A class run by its **regular** instructor stores that instructor on the *slot* (`class_schedule_slots.instructor_person_id`), so it produced **no row** and the instructor never appeared (Mason: "created recurring classes… checked in two people… shows nothing"). Also the attendee count only matched `bookings.status = 'confirmed'`, so checked-in (`attended`) people counted as 0.
  - **#1** — rewrote the query so each occurrence's teaching credit is `COALESCE(event_session_staffing override, slot instructor)`: a `UNION` of (a) staffing rows with role instructor/substitute and (b) the slot instructor for sessions with **no** teaching-staffing row (`NOT EXISTS`). A substituted-away regular instructor gets no credit for that occurrence (no double count).
  - **#2** — attendees now count `bookings.status IN ('confirmed','attended')`; waitlisted/cancelled still excluded.
  - Tests: **staffing_helper_test.cpp** +3 (slot-instructor fallback w/ attended+confirmed, substitute overrides slot instructor, attended counted on the staffed path); **admin_instructor_load_test.cpp** +1 end-to-end (materialize an occurrence with **no** staffing → slot instructor credited, confirmed+attended counted). All prior GetInstructorLoad tests remain valid (they use slot-less sessions + explicit staffing → source A path unchanged). *(Backend build+run is Mason's step.)*
  - Frontend follow-through: the "Confirmed Attendees" column → **"Attendees"** and the tooltip/description reworded (the number now includes checked-in people). Deferred (bigger, separate): the report still only counts **materialized** `event_sessions`, so a purely future recurring class with no bookings/check-ins still won't show until an occurrence is ensured — a derive-occurrences-from-slots change.
...
- [x] **Sortable columns**: clickable header buttons for **Instructor** (last, then first name), **Sessions**, and **Attendees**. Same-column click flips asc/desc; a new column resets to its natural default (name A→Z, counts high→low). Active column shows a ▲/▼ arrow. Default sort is **attendees descending** (busiest instructor first). Raw `rows` kept separate from the sorted `displayRows` view.
- [x] **CSS-only "Relative load" bar column** (no chart dependency — resolved OQ-P14-1): a horizontal bar per instructor, width normalized to the busiest instructor's attendee count (`barPercent`), with a `matTooltip` ("{name}: {N} attendees across {M} sessions"). Zero-attendee / all-zero result guarded (0% width, keeps a 2px stub).
