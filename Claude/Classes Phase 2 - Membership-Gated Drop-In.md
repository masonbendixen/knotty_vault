...
- [~] **Deferred.** The calendar (`pages/calendar/`) is **entirely mock-driven** — `CalendarService` returns `mockCalendarResponse()` with a `TODO replace with API call`, and `CalendarEvent` carries only `{id,title,startTime,endTime,location}` (no class / price / membership fields). It is not wired to `visible_event_sessions` or any backend. Chip-labeling sessions by "Included / Tier-priced / Members only" first requires connecting the calendar to the real session API — a calendar↔API integration well outside Phase 2's membership-gating scope. Revisit when the calendar is wired to live data — now tracked as **[[Classes Phase 16 - Stretch Items]] §4 (CAL-1)**, which consolidates this with the Phase 10/11/13 calendar deferrals. (The membership pricing/visibility surface is already delivered via the class catalog + `visible_event_sessions`, which now carry `class_*` + `can_book` + price.)
...
- [x] **Add**: a single form — searchable **people picker** (`mat-autocomplete` over `getFkOptions('people', …)`, `displayWith` for clean labels, excludes people already instructors) + bio textarea + a **`photo-upload` control (`deferUpload`)** right in the form. On Create → `addItemFetchPrimaryKey('instructors', { person_id, bio })`, then `@ViewChild(PhotoUploadComponent).uploadPendingPhoto('instructors', newId)` flushes the chosen photo to the new row, then back to the list. Guarded: Create disabled / errors without a chosen person; a photo-only failure still keeps the created instructor. *(Reworked 2026-06-02 from the original two-step "create then add photo" — photo upload is now visible up-front in the Add form per Mason's feedback.)*
