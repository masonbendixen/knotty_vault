...
- [x] `GetWeeklySchedule(tx, weekStartUs)` — `weekStartUs<=0` ⇒ current week (UTC-midnight Sunday on/before `now_us()` via local `TruncToUtcMidnight`/`DayOfWeekUtc`). For each `ClassCatalogHelper::GetActiveClasses`, `ClassScheduleHelper::GetDerivedSessionsForRange(classId, weekStart, +7d)` (so dark days are skipped for free); enriches each derived session: `className` (from the catalog entry), `classHasPhoto` (`TableItemPhotos::HasPhoto("classes", classId)`), `durationMinutes` (`(endUs-startUs)/60s`), `instructorName` (`People::GetPersonById`), `requirementsDescription`, and predecessor (slot → `predecessor_class_schedule_slot_id` → walk slot→impl→instance→class for the cross-class name + day/start). **Cancelled occurrences are skipped.** No CRUD SQL in business logic — all via table helpers (only `SELECT now_us()`, the accepted scalar pattern).
...
- [x] Tests (`weekly_schedule_helper_test.cpp`, 8 cases): Mon+Wed land on the right days with duration/className; instructor name resolved / empty when none; requirements description joined from groups (empty group skipped) / empty for an open class; cross-class predecessor resolved (`requires_attending`); `weekStartUs<=0` defaults to the current-week Sunday (UTC-midnight, ≤ now); a future-windowed instance contributes no slots to the queried week (dark). Registered both files in `CMakeLists.txt`.
...
- [x] **Public route `/schedule`** → new `OurScheduleComponent` (`pages/public/our-schedule/`, standalone, `SharedModule`; route registered in `pages/public/public.routes.ts`). Renders a **Sun–Sat grid** (7 columns desktop, stacked on mobile); each day lists its class slots as cards showing: class **photo thumbnail** (`/api/get_scaled_photo/classes/<id>/…` when `has_photo`, with a placeholder otherwise), **class name**, **instructor**, **duration**, **requirements inline description**, and a **"Requires attending: {class} · {day} {time}"** line when set. Prev/next/today week navigation via `week_start_us`. Empty-day + empty-week states.
...
- [ ] **C++ (for Mason to build/run)**: DB/table-helper inline_description round-trip · `WeeklyScheduleHelper` (days / enrichment / predecessor / requirements join / dark days / week-start default) · `/api/schedule/week` endpoint + the §6.6 inline_description read. *(Tests written; not built locally per the no-build-server rule.)*
