...
- [x] **Startup fix (2026-06-11, found by Mason first run since the AWS migration):** the tool died with "invalid stoul argument" — `CreateContext` still built the plain one-arg `SecretsHelper`, so after Phase 8.1's at-rest encryption every `LookupSecret` returned raw `v1:...` ciphertext (the mail-port `stoul` threw; the Square client was silently getting an encrypted token). Fixed by mirroring the server's bootstrap: plain helper → read `server_production_mode` → `MakeSecretsAtRest(isProd)` (dev-key fallback) → re-create the helper with the decryptor. **Same audit found a real production bug:** `EventReminderHelper::SendPendingReminders` constructed its own plain `SecretsHelper` inside business logic — post-migration the `event_reminder_hours` lookup returned ciphertext that poisoned the reminder SQL (uncaught — hourly event reminders have been broken since the migration ran). Fixed via DI: new `(db, mail, secrets)` ctor; the endpoint passes `GetSecretsHelper()`, the `ser` test-helper command passes `ctx.secretsHelper`, 2-arg ctor falls back to the 24h default with NO DB read. New test `InjectedSecretsWidenTheWindow` (48h secret reminds a 36h-out event; null-secrets fallback skips it). Audited all remaining one-arg `MakeSecretsHelper` call sites: server main (intentional bootstrap) and `create_database` (seeds plaintext pre-migration) are correct.

### 9.6 Frontend ✅ DONE (code; end-to-end verify is Mason's manual step)
- [ ] Verify end-to-end with seeded data: seed one person via `sca`, open the next occurrence of that slot in `/staff/class-checkin`, confirm they appear under **Recent Attendees** with their "Last attended" date and one-click check-in works (this is the §9 acceptance test — needs the rebuilt server + test helper).
- [x] `checkin.types.ts`: `last_checked_in_us?: number` on `CheckinCandidate` (comment flags it as the REAL-instant exception); `ServerAccessNetwork.normalizeCheckinCandidate` coerces with `Number()` when present, leaves `undefined` otherwise; mock seeds it on both `history` candidates (1w/2w before the session) + mock spec `carries last_checked_in_us on history rows only`.
- [x] `class-checkin.component`: dim italic "Last attended {Mon DD}" subtitle on rows carrying the field. `formatLastAttended` formats in **browser-local** time with a "don't 'fix' this to UTC" comment (the one real-instant timestamp on the page).
- [x] Component spec (3 new): exactly one label renders (the history candidate) with the self-consistent formatted date; zero labels when the field is absent; format-shape test documenting the browser-local choice. Full Angular suite: 2548/2548.
