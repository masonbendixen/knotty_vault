...
- [x] `TenantResources` lazily builds the tenant's `SecretsHelper` from its own `DatabaseHelper` (`components/platform/business_logic/tenancy/tenant_resources.cpp`); `EndpointAuthHelper::GetSecretsHelper()` re-points from the global to the tenant resources. Keep the **global** at-rest master key `HONUWARE_SECRET_KEY` (it decrypts every tenant's `config_secrets.value` — §1.6 "stays global"). `SecretsHelper` needs no interface change — only a different `DatabaseHelper` (verified by construction in componentization 1.3/2.2). *(As-built: `MakeDefaultTenantResources(context, secretsAtRest)` overload + `TenantResources::secretsHelper` field; accessor falls back to global when null.)*
- [x] Tests: a `SecretsHelper` built against tenant-A resources reads tenant-A values; the cross-tenant-bleed case runs on the Phase 3.4 physical-isolation pair (two helpers → two databases → distinct `config_secrets` values). *(`TenantResourcesTest.SecretsAtRestFactory…`, `TenantPhysicalIsolationTest.PerTenantSecretsHelpersAreIsolated`, echo `secrets_is_tenant`.)*

### 4.2 Per-tenant `SquareClient` **[app]**
- [x] New `src/business_logic/app_tenant_resources.{h,cpp}` — `KnottyTenantResources : Tenancy::TenantResources` adding a lazy `GetSquareClient()`: reads `Secrets::kSquareAccessToken` + `kSquareEnvironment` (`business_logic/app_secret_keys.h`) from the **tenant** secrets (4.1) and builds via `honuware_square`'s `MakeSquareClient(httpClient, token, isSandbox)` — sandbox/prod **per tenant**. Wire the registry factory in `src/main.cpp` to return it (the Phase 2.2 extension point). *(As-built: built in a `BuildAppServices(txn)` override called by the framework's `EnsureServices` during `Initialize` — see completion note on why lazy-during-Initialize, not lazy-in-accessor.)*
- [x] `AppEndpointAuthHelper::GetSquareClient()` (`src/endpoints/app_endpoint_auth_helper.cpp`) re-points from the process-wide `WebApp` service registry to the request's `KnottyTenantResources` (downcast of `GetTenantResources()`); the 21 Square-using endpoints keep their `helper.GetSquareClient()` call sites unchanged. Retire the startup `webApp.SetService<Square::SquareClient>(...)` registration once all call sites go through the tenant path. *(Downcast-then-fallback to the WebApp registry; the startup registration is KEPT as the fallback for endpoint tests + tenants without Square — retiring it is left as later cleanup.)*
- [x] Tests: tenant on sandbox vs tenant on prod yield correctly-configured clients (per-tenant secrets on the isolation pair); `EndpointTestHelper` registers its `MakeTestSquareClient` double into the default fixed tenant's resources so the ~15 existing Square endpoint tests pass unchanged; `app_tenant_resources_test.cpp` for the factory/laziness. *(Square endpoint tests pass via the WebApp-registry fallback — framework `TenantResources` isn't a `KnottyTenantResources` so the downcast misses; `app_tenant_resources_test.cpp` covers sandbox/prod/unconfigured. SquareClient exposes no env accessor, so both branches assert non-null.)*

### 4.3 Per-tenant `MailHelper` **[hw]**
- [x] `TenantResources` lazily builds `MailHelper` per tenant (`MakeMailHelper(Transaction&, tenantSecrets)` — the services-layer constructor already reads SMTP config through the given secrets helper); the framework mail accessor re-points to it. Sender identity flows automatically: every From-address site already calls `::Mail::LoadSenderAddress(secrets, txn)`, so handing it the **tenant** secrets helper completes per-tenant identity with zero call-site edits. *(As-built: `TenantResources::EnsureServices(txn)` builds mail once via `std::call_once`, called inside `Initialize`'s transaction; `GetMailHelper()` re-points with global fallback; tests inject the mail double so `EnsureServices` skips the real SMTP build.)*
- [x] Tests: SMTP config + sender identity reflect the tenant (isolation pair, distinct `mail_sender_*` values); existing mail tests keep passing via the test double registered in the default fixed tenant. *(`TenantResourcesTest.EnsureServicesBuildsMailHelperFromTenantSecretsOnce` + `…KeepsInjectedMailHelper`, echo `mail_is_tenant`; the ~injected test double keeps every existing mail test green.)*

### 4.4 `ServerConfig` global vs per-tenant **[hw]**
- [x] Smaller than originally planned (verified 7/21/2026): the `Auth::ServerConfig` singleton (`components/platform/business_logic/auth/server_config.{h,cpp}`) **already stores only the global bits** (`prodMode_`/`testMode_`) — website address and CORS are read from secrets at `Initialize` and applied to the middleware, not stored. So: keep the singleton as the deployment-global config (its startup read runs against the deployment's **primary** DB — knottyyoga's own, tenant #1 — unchanged sequence in `main.cpp`); no rename needed unless clarity wants it. The real work is the two per-tenant behaviors below. *(Confirmed — kept as-is.)*
- [ ] Rework CORS: each tenant is same-origin behind its own CloudFront, so prod CORS is effectively a no-op; replace the single pinned-origin registration with a dynamic check validating the request `Origin` against the **resolved tenant's** origin, keeping the dev `HONUWARE_DEV_CORS_ORIGIN` path. Remove the `ServerConfig` global-singleton assumption. **⟶ DEFERRED (hygiene, not functional):** documented in `server_config.cpp` — every tenant is same-origin behind its own CloudFront so the browser never issues a cross-origin request and CORS never engages; the startup global pin is a no-op in the real topology. A dynamic per-tenant ACAO check needs the resolved tenant's origin (a per-tenant secret) inside the CORS middleware; deferred until a genuinely cross-origin tenant appears.
- [x] Tests: cookie `Domain`/`Secure` derive from the tenant's website address (extend the existing `SessionTest.InitializeFromLoginProdMode…` coverage to assert per-tenant domain on the isolation pair); CORS accepts the tenant origin and rejects others. *(Cookie done — `TenantPhysicalIsolationTest.PerTenantCookieDomainDerivesFromTenantWebsite`; it flows automatically from 4.1 since `BuildAuthCookieProperties` reads `kWebsiteAddress` through the per-tenant SecretsHelper. CORS accept/reject test deferred with the CORS rework above.)*
...
> **✅ Phase 4.1–4.4 complete — 2026-07-23** (the honuware-touching work). Both Linux docker clients green in co-dev: **honuware full 1367** (+6 over Phase 3), **app co-dev full 4517** (+9). All existing tests pass with the tenant re-points in place (accessors fall back to the global when a service isn't wired, so tests using the framework test-double resources are unaffected).
> **Key as-built decision — services that need a transaction are built during `Initialize`, not lazily in the accessor.** `GetMailHelper()`/`GetSquareClient()` are called *inside* the endpoint's `RunInTransaction` on the tenant's pool-of-1 connection (verified: `admin_cancel_session.cpp`), so lazy-building them there would re-enter the pool and deadlock. Instead `TenantResources::EnsureServices(txn)` (a `std::call_once`, virtual `BuildAppServices` hook for the app's Square) runs inside `Initialize`'s session-init transaction — which completes *before* any endpoint transaction. Secrets need no transaction, so they're built eagerly in the factory.
> **Non-breaking via fallback:** every re-pointed accessor returns the tenant service *if present, else the global*. Tests inject the test doubles (secrets, mail) into the fixed tenant's resources so `EnsureServices` skips real builds; Square falls back to the WebApp registry (the test double) because `EndpointTestHelper` uses framework `TenantResources`, not `KnottyTenantResources`.
> **Deferred within Phase 4:** (a) **4.4 CORS** dynamic rework — a no-op in the same-origin CloudFront topology (documented in code); (b) **4.5** — app-only, 30 `*_mail.cpp` brand substitution. 4.5 touches NO honuware code, so it needs no honuware bump and can land after this bump point.
> **⇑ bump point pending [Mason]:** push honuware Phase 4.1–4.4 → CI green → [Claude] re-pin app `GIT_TAG` d45ad72→new + verify pinned SHA → [Mason] commit app (`CMakeLists` re-pin + `src/main.cpp` + `src/business_logic/app_tenant_resources.*` + `src/endpoints/app_endpoint_auth_helper.cpp` + `src/business_logic/CMakeLists.txt`).
