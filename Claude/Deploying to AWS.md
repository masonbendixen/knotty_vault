...
- [ ] ~~(Optional) CloudFront access logs~~ — **BLOCKED on the Free plan, and not worth unblocking (9/25).** Clicking distribution → **Logging** shows *Standard logging* behind a **lock icon with an "Upgrade to Pro" button**: access logging is a **Pro-plan feature**, and Phase 4.6 chose **Free** deliberately.
	- ⚠️ **This doc previously said "Free aside from S3 storage" — that is wrong.** It was true when CloudFront billed purely per-usage; the plan tiers changed it. The real price is **$15/month** for Pro (per the plan selector recorded in 4.6), i.e. ~$180/year.
	- **Decision: stay on Free, do not upgrade for logs.** What you keep for nothing: the **Monitoring** tab (requests, bytes, 4xx/5xx rates) covers "is it working" and "is traffic growing"; `/knottyyoga/ec2` already holds every `/api/*` request via the Crow logs.
	- **The one real gap — frontend page views — has a free and better answer than access logs anyway:** client-side analytics (GoatCounter, Plausible, or a counter endpoint on our own API). It sees edge-cached hits, which origin logs never do, and needs no Athena to read. Worth its own item if page-level numbers are wanted.
	- **What Pro would genuinely buy:** cache-hit-ratio debugging and edge-level bot/abuse forensics. Neither is a soft-launch concern. Free → Pro is a non-destructive upgrade whenever one becomes one — revisit then, and the bucket/lifecycle steps below still apply.
	- *Procedure kept for that day:* (1) check the **Logging** tab to see which mode Pro offers — legacy vs standard-logging-v2 decides one bucket setting; (2) S3 → Create bucket `knottyyoga-cf-logs`, `us-west-2`, General purpose, Block Public Access all on, Versioning **Disable**, SSE-S3 — and **Object Ownership → ACLs enabled (*Bucket owner preferred*)** if and only if the mode is legacy, since CloudFront writes each file with an ACL grant and the failure never mentions ACLs (changeable later under Permissions → Object Ownership); (3) lifecycle rule `expire-after-90-days`, all objects, *Expire current versions* after 90 days, plus *Delete incomplete multipart uploads* after 7; (4) enable on the distribution with prefix `cf/`; (5) first files land in 10–60 minutes, not instantly. Reading them means gzipped TSV, or Athena over the bucket for anything regular.
...
- [x] **No custom watchdog process needed**. The custom `knottyyoga_helper` watchdog mode from `Scheduled Jobs.md` is dropped from scope. `knottyyoga_helper` retains only the scheduled-jobs runner (subscription billing, reminders). ✅ 2026-09-25
