...
- [ ] (Optional) **CloudFront access logs → a dedicated S3 bucket.** Still deferred, deliberately. *What they add over metrics:* per-request detail — URL, referer, user-agent, edge location, cache hit/miss. **The gap is specific and real: the EC2 never sees frontend traffic**, because S3-served pages and cached assets are answered at the edge, so the Crow logs in `/knottyyoga/ec2` only ever show `/api/*`. Today there is no way to tell how many people loaded the site. Enable when there is a question they would answer ("why is the cache hit ratio low", "what is hammering us"), not before. Procedure when that day comes:
	1. **Look at CloudFront's Logging tab FIRST** — `E23TY4IAUHGM6H` → **Logging** — to see which mode this console offers. It decides one bucket setting below, and checking costs nothing.
	2. **Create the bucket.** S3 console → **Create bucket** (direct: `https://s3.console.aws.amazon.com/s3/buckets?region=us-west-2`). Same form as `knottyyoga-ui-prod` in 4.6, different answers:
		- **Region** `US West (Oregon) us-west-2`; **Bucket type** General purpose; **Name** `knottyyoga-cf-logs`
		- **Object Ownership** — the radio group holding the ACL choice. ⚠️ **"Standard logging (legacy)" requires *ACLs enabled* → Bucket owner preferred**, because CloudFront writes each file with an ACL grant; new buckets default to **ACLs disabled**, and the failure is a permissions error that never mentions ACLs. **Standard logging v2** (delivery to S3 / CloudWatch Logs / Firehose) uses vended-log delivery and works with the default. *Not a one-way door either way* — bucket → **Permissions** → *Object Ownership* → Edit changes it later in seconds.
		- **Block Public Access** all four ticked; **Versioning** Disable (disposable logs — versioning just doubles the storage); **Default encryption** SSE-S3.
	3. **Lifecycle rule, before enabling logging** — bucket → Management → Create lifecycle rule → all objects → *Expire current versions after 90 days*. Without it the files accumulate forever.
	4. **Enable:** CloudFront → `E23TY4IAUHGM6H` → **Logging** tab (older consoles: General → Settings → Edit → *Standard logging*) → On → the bucket → prefix `cf/`.
	5. **Delivery is batched** — first files appear in 10–60 minutes, not immediately. Do not conclude it is broken at five minutes.
