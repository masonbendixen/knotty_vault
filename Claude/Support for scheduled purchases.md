...
- **Bidirectional hole prevention**: Valid start times within a free window are spaced at `min_slot` intervals (`base_duration + buffer`), starting from the window start. This prevents unusable gaps both before AND after a booking. Example: Provider available 8:00am-4:00pm, 60min base variant, 5min buffer → valid starts are 8:00, 9:05, 10:10, etc. 8:05am or 8:30am are NOT valid because they would leave a gap at the start too small for any variant.
- **End-of-window best-fit**: The last slot before a window end may leave a trailing gap (unavoidable). In this case, only the **longest variant that fits** is offered — shorter variants are rejected because they'd waste more provider time. Example: 110min remaining → only 90min offered (not 60min which would leave 50min unused, nor 120min which doesn't fit).
